...
class GRUCFModelRegress2:
    ...
    def gruRegression(self, xData, yData):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(xData)):
            ...
            model = Sequential()
            # GRU层
            model.add(GRU(units=30, input_shape=(self.memory + 1, self.numVar), activation='tanh', return_sequences=True))
